...
3. **Trace recalled anti-patterns** (best-effort)
   - If the failing code came from memory recall → `mcp__dense-mem__trace_memory(...)` to find provenance.
   - If you stored evidence that proved wrong → `mcp__dense-mem__retract_evidence(...)`.
