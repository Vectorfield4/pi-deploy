...
- **Anti-pattern recall**: `dense_mem_recall_memory({ query:"<goal> project:<project> anti-pattern" })` — use the project's prior failures to avoid repeating them.
...
- If QA proves a recalled solution was wrong, find the offending record with `dense_mem_trace_memory({ ... })`.
- If the coder profile owns the record, retire it with `dense_mem_retract_evidence({ evidence_ids:[...], reason:"...", idempotency_key:"..." })`.
...
- For relationship corrections, use `dense_mem_correct_relationship({ ... })` (only if the caller owns the active Relationship).
