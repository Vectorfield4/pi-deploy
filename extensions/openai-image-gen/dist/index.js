...
function inferApi(provider, apiField) {
    if (apiField === "openai-images" || apiField === "openai-chat-image" || apiField === "google-generative-ai-image")
        return apiField;
    if (provider === "openai")
        return "openai-images";
    if (provider === "google")
        return "google-generative-ai-image";
...
async function generateViaOpenAIImages(cfg, prompt, aspectRatio, signal) {
    ...
    if (!res.ok)
        throw new Error(`OpenAI images failed (${res.status}): ${await res.text()}`);
    const json = (await res.json());
    if (!json.data?.[0]?.b64_json)
        throw new Error("OpenAI returned no image data");
    return { data: json.data[0].b64_json, mimeType: "image/png" };
}
async function generateViaChatImage(cfg, prompt, aspectRatio, signal) {
    const body = {
        model: cfg.model,
    ...
    if (!res.ok)
        throw new Error(`Chat image failed (${res.status}): ${await res.text()}`);
    const json = (await res.json());
    const images = json.choices?.[0]?.message?.images;
    if (!images?.[0]?.image_url?.url)
        throw new Error("No images in response (model may not support image generation)");
...
async function generateViaGoogleImage(cfg, prompt, signal) {
    ...
    if (!res.ok)
        throw new Error(`Google image failed (${res.status}): ${await res.text()}`);
    const json = (await res.json());
...
async function generate(cfg, prompt, aspectRatio, signal) {
    switch (cfg.api) {
...
export default async function (pi) {
    pi.registerTool({
        ...
        description: "Generate an image from a text prompt via OpenAI-compatible endpoints (OpenRouter, OpenAI direct, Google). Provider/model/baseUrl come from env OPENAI_IMAGE_*, then .pi/settings/pi-openai-image-generation.json, then env IMAGE_API_URL/IMAGE_API_KEY. Primary tool: hf_generate_image.",
        ...
        async execute(_toolCallId, params, signal) {
            ...
            if (!cfg.apiKey)
                throw new Error("No API key: set OPENAI_IMAGE_API_KEY or IMAGE_API_KEY env");
            if (!cfg.baseUrl)
                throw new Error("No base URL: set OPENAI_IMAGE_BASE_URL or IMAGE_API_URL env");
            ...
            try {
                ...
                return {
                    content: [
                        { type: "text", text: `Generated image (${cfg.provider}/${cfg.model}, ${cfg.api}, ${aspect}).${savedPath ? ` Saved: ${savedPath}` : ""}` },
